...
4. **Export** (`export.py`) — writes `data.json`, `releases.csv`, `notes.json` to `data/cooked/`
5. **Render** (`render.py`) — assembles the dashboard from template partials and injects computed data. First resolves `{{INLINE:filename}}` markers to inline CSS/JS files, then replaces data placeholders with HTML-safe JSON
...
CI runs tests on every push/PR (`test.yml`) and deploys `public/` to GitHub Pages hourly, on push to main, and via `workflow_dispatch`. GitHub disables scheduled workflows after 60 days without repo activity; `make github-actions-refresh` resets that timer on all scheduled workflows and re-enables any already disabled.
